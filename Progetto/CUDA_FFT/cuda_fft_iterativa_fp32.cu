/*
    QUESTA È UNA IMPLEMENTAZIONE NON FUNZIONANTE!
    penso che la causa sia dovuta alla sincronizzazione solo a livello di blocco, e non a livello di stadio, che synchthreads() permette.
*/
...
__global__ void fft_kernel(complex *input, complex *output, int N, int num_stadi) {
    ...
    for (int stadio = 1; stadio <= num_stadi; stadio++) {
        ...
        // questo calcolo è ripetuto all'interno di tutti i thread in questo stadio -> OTTIMIZZARE!
...
double fft_iterativa_cuda(complex *input, complex *output, int N) {
    ...
    // Data la simmetria, 1 thread gestisce due campioni della trasformata
    int num_threads = N/2;
    ...
    */
    ...
    fft_kernel<<<num_blocks, threads_per_block>>>(d_input, d_output, N, num_stadi);
...
int main(int argc, char **argv) {
    ...
    */
    ...
    printf("\tFFT gpu_ref_fft_samples %f\n", gpu_ref_fft_samples[0].real);
    ...
    printf("\tIFFT complex_signal_samples %f\n", complex_signal_samples[0].real);
    convert_to_short(complex_signal_samples, signal_samples, num_samples);
    printf("\tIFFT signal_samples %d\n", signal_samples[0]);
